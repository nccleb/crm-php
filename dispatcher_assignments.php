...
?></textarea>
        ...
        <div id="trackingSection" style="background: lightyellow; padding: 15px; border: 2px solid blue;">
            ...
            <div style="margin-top: 10px; background: #e8f5e8; padding: 10px; border: 1px solid green;">
...
function setupWhatsAppForDriver(driverId, driverName) {
    ...
    let trackingUrl = currentTrackingUrl + '/driver_mobile.php?driver_id=' + driverId;
    ...
    const whatsappMessage = createWhatsAppMessage(driverName, trackingUrl);
...
function createWhatsAppMessage(driverName, trackingUrl) {
    // WhatsApp-optimized format for clickable links
    let message = `Hi ${driverName}! 👋\n\n`;
    message += `🚚 Please start location tracking:\n\n`;
    
    // Ensure clean URL format for WhatsApp
    const urlParts = trackingUrl.split('?');
    const baseUrl = urlParts[0];
    const params = urlParts[1];
    const cleanUrl = baseUrl + '?' + params;
    
    message += `${cleanUrl}\n\n`;
    message += `📱 Instructions:\n`;
    message += `1. Tap the link above\n`;
    message += `2. Tap "Start Tracking"\n`;
    message += `3. Allow location access\n\n`;
    message += `Thank you! 🙏`;
...
function copyTrackingUrl() {
    document.getElementById('trackingUrl').select();
    document.execCommand('copy');
    alert('Raw tracking URL copied!');
}
