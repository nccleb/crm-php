	...
	//echo substr($tab[$ca],-16);
      ...
      $a=substr($tab[$cb],19,3);
	    ...
	    $cb;
			...
			if(preg_match("/D0/" ,$tab[$cb])==1){
            if(	substr_count($tab[$cb],"D0")==2){			
            if($s=="D0"){
...
if (mysqli_connect_errno()) {
  ...
  exit();
	...
	WHERE NOT EXISTS
    (SELECT id FROM billing WHERE email = 'john@mmm.com')"


);
  
  
     $stmt->bind_param("siis",$c,$a,$du,$s);
...
$to = "nccleb@gmail.com,info@nccleb.com";
$subject = "My subject";
$txt ="hello".$c;
