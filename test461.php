...
$curlResponse = curl_exec($curlHandle);
 ...
 $fields = '{ 
  ...
  curl_setopt($curlHandle, CURLOPT_RETURNTRANSFER, true);
    ...
    echo  $ch = $ch['response']['channel'][0]['channel'];




    if(isset($_POST['sta'])&&isset($_POST['end']))
    {
...
curl_setopt($curlHandle, CURLOPT_RETURNTRANSFER, true);
 $curlResponse = curl_exec($curlHandle);
  ...
  $queuechairman = $ch['queue_statistics'][0]['total']['queuechairman'];
  //$queue = $ch['queue_statistics'][0]['total']['total']."<br>";
  $total_calls = $ch['queue_statistics'][0]['total']['total_calls']."<br>";
  //$answered_calls = $ch['queue_statistics'][0]['total']['answered_calls']."<br>";
  //$answered_rate = $ch['queue_statistics'][0]['total']['answered_rate']."<br>";
  $avg_wait = $ch['queue_statistics'][0]['total']['avg_wait'];
  $avg_talk = $ch['queue_statistics'][0]['total']['avg_talk'];
  $vq_total_calls = $ch['queue_statistics'][0]['total']['vq_total_calls'];
  $abandoned_rate = $ch['queue_statistics'][0]['total']['abandoned_rate'];
...
<table>
<tr><th>queuechairman</th><th>total_calls</th><th>abandoned_rate</th><th>avg_wait</th><th>avg_talk</th><th>vq_total_calls</th></tr>
<?php 
	 
	  
	
			
			   echo	"<tr><td>"."<a>" .$queuechairman. " </a>"."</td><td>"."<a  \">" .$total_calls . " </a></li>\n"."</td>"."<td>"."<a>".$abandoned_rate."</a>"."</td>"."<td>"."<a>".$avg_wait."</a>"."</td>"."<td>"."<a>".$avg_talk."</a>"."</td>"."<td>"."<a>".$vq_total_calls."</a>"."</td>"."</tr>";
