...
if ($all_fields_present) {
        ...
        while($lig = mysqli_fetch_assoc($req6)) {
            ...
            if (!empty($tes)) {
                if ($tes == $lig['number'] || $tes == $lig['inumber'] || 
                    ...
                    echo "<script>location.replace('http://192.168.20.109/test275.php?page=" . urlencode($naa) . "&page1=" . urlencode($idf) . "&page2=" . urlencode($inc) . "');</script>";
                    exit();
                }
            }
    ...
    if ($test == 0) {
        ...
        if (r == true) {
            location.replace('http://192.168.16.102/test275.php?page=" . urlencode($naa) . "&page1=" . urlencode($idf) . "&page2=" . urlencode($incc) . "');
        } else {
            window.close();
        }
        </script>";
